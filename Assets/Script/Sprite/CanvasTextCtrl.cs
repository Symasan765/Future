...
public class CanvasTextCtrl : MonoBehaviour {
	...
	IEnumerator SpriteUpdate()
	{
		...
		for (float timeCnt = 0.0f; timeCnt < ExpansionTime; timeCnt += Time.deltaTime)
		{
			...
			rectTransform.localScale = Vector3.Lerp(Vector3.zero, ExpansionSize, t);
			yield return null;
		}   //拡大終わり
		...
		for (float timeCnt = 0.0f; timeCnt < ReducedTime; timeCnt += Time.deltaTime)
		{
			...
			rectTransform.localScale = Vector3.Lerp(ExpansionSize, m_InitialScale, t);
			yield return null;
		}   //縮小終わり
		...
		for (float timeCnt = 0.0f; timeCnt < endTime; timeCnt += Time.deltaTime)
		{
			...
			rectTransform.localScale = Vector3.Lerp(m_InitialScale, Vector3.zero, t);
			yield return null;
		}   //縮小終わり
